...
wxBEGIN_EVENT_TABLE(MainFrame, wxFrame)
   ...
   /* EVT_SCROLL_CHANGED(ID_VolumeSlider, MainFrame::OnVolumeSliderScroll)
    EVT_SCROLL_CHANGED(ID_FrequencySlider, MainFrame::OnFrequencySliderScroll)*/
...
wxMenuBar* MainFrame::buildMenuBar()
{
    ...
    menuFile->Append(ID_OpenFile, "&Open...\tCtrl-O", "Will Open the Open File Dialog");
...
wxPanel* MainFrame::buildMainPanel()
{
    ...
    buildFrequencyVolumeSection(panel);
    buildButtonsSection(panel);

    //// Volume Control Section
    //wxStaticBox* volumeBox = new wxStaticBox(panel, wxID_ANY, "Volume", { 10,10 }, { 170,60 });
    //volumeSlider = new wxSlider(volumeBox, ID_VolumeSlider, 0, 0, 100,
    //    wxPoint(5, 20), wxSize(140, -1));

    //volumeText = new wxStaticText(volumeBox, wxID_ANY ,"0", wxPoint(150, 25));


    //// Frequency Control Section
    //wxStaticBox* frequencyBox = new wxStaticBox(panel, wxID_ANY, "Frequency", { 190,10 }, { 190,60 });
...
wxStaticBox* MainFrame::buildFrequencyVolumeSection(wxPanel* parent)
{
    // Frequency Control Section
...
void MainFrame::OnPlayButtonClick(wxCommandEvent& event)
{
    ...
    wxString countStr = wxString::Format(wxT("%i"), ++clickCount);

    //mainEditBox->SetLabelText(countStr);
}
